...
const Account = () => {
	...
	const [joinedDate, setJoinedDate] = useState<string | null | undefined>(null);
		...
		if (id) {
			const fetchData = async () => {
				try {
					...
					if (postError) {
						console.error("Error fetching post count:", postError.message);
					}
					...
					if (tagsError) {
						console.error("Error fetching tags count:", tagsError.message);
					}
				} catch (err) {
	...
	const fetchMorePosts = async () => {
		...
		try {
			...
			if (data) {
				...
				setError(error.message);
				console.error("Error fetching user's profile posts:", error.message);
			}
		} catch (error) {
		...
		<main className="">
			...
			<div className="max-w-5xl px-3 py-8 mx-3 mb-4 rounded bg-background md:px-4 md:mx-4 lg:mx-auto text-foreground">
										...
										{followerProfiles.length ? (
											...
											>
												{followerProfiles?.map((follower) => {
													const { display_pic, display_name, username, id } =
														follower;
															...
															>
																<div className="flex items-center rounded">
																	...
																	<div className="flex flex-col pl-5 transition hover-bg-foreground/5">
																		<h1 className="text-lg">{display_name}</h1>
																	...
																	<button
																		...
																		onClick={() =>
																			handleFollowUnfollowForMutualFollowers(
																				id as string,
																				isFollowerMutual[id as string] || false, // Pass isFollowerMutual for this specific user
																				username
																			)
																		}
																	>
																		{isFollowerMutual[id as string]
																			? "Unfollow"
																			: "Follow"}
																	</button>
																)}
																	...
																	<button
																		...
																		onClick={() =>
																			handleFollowUnfollowForMutualFollowing(
																				id as string,
																				isFollowingMutual[id as string] ||
				...
				<div className="flex flex-col gap-8 py-8 md:flex-row md:flex-wrap md:justify-center">
						...
						{joinedDate ? (
							<p>
								<div className="flex items-center gap-2">
									<p>Joined on {dateFormatter.format(new Date(joinedDate))}</p>
									<p suppressHydrationWarning className="text-xs opacity-75">
										{" "}
										(
					...
					{showEmail ? (
						email ? (
							<Link to={`mailto:${email}`} className="flex gap-4 text-sm">
				...
				>
					...
					<div className="flex flex-col pt-3 gap-7">
						<p className="flex items-center gap-3">
							...
							<p>{postCount ? `${postCount} posts published` : "Loading..."}</p>
						</p>
						<p className="flex items-center gap-3">
						...
						<p className="flex items-center gap-3">
							...
							<p>{tagsCount ? `${tagsCount} topics followed` : "Loading..."}</p>
						</p>
					</div>
				</div>
				...
				<div className="col-span-1 px-1 mx-2 mb-4 md:mx-0 text-foreground md:sticky md:top-[100px] hidden md:block ml-3">
					...
					<div className="flex flex-col p-3 pt-3 mb-4 rounded gap-7 bg-background">
						<p className="flex items-center gap-3">
							<ScrollText className="opacity-70" />
							<p>
								{postCount ? `${postCount} post(s) published` : "Not available"}
							</p>
						</p>
						<p className="flex items-center gap-3">
							<MessageSquare className="opacity-70" />
						...
						<p className="flex items-center gap-3">
							<Hash className="opacity-70" />
							<p>
								{tagsCount ? `${tagsCount} topics followed` : "Not available"}
							</p>
						</p>
					</div>
				</div>
